...
## 🚗 1. Drive System Initialization

Defines the drivetrain configuration and steering mechanism.
...
Ultrasonic sensors are used for environmental awareness.
```python
...
```
Purpose
 - Stores desired robot orientation
 - PID derivative memory
 - Enables continuous heading correction across turns

## 🔧 4. Utility Functions

Clamp Function (Safety Limiter)
```python
def clamp(v, mn, mx):
...
Purpose:
 ...
 - Ensures stable mechanical behavior
...
```
Purpose:
 - Keeps gyro values within stable range
 - Prevents overflow drift issues
...
```
Purpose:
 - Reduces ultrasonic noise
 - Stabilizes wall detection
 - Improves reliability of gap detection
   
## 🎯 5. PID-Based Driving (Core Control Loop)
...
```
Purpose
 - Executes controlled 90° left turn
 - Updates global navigation reference
 - Resets PID state after rotation

## 🚦 7. Main Navigation Loop
...
|----------|----------------|
| `gyro_follow()` | Continuous PID navigation |
| `turn_left_90()` | Left intersection handling |
| `turn_right_90()` | Right intersection handling |
| `filtered_distance()` | Sensor stabilization |
